...
type VisitaRow = {
  ...
  // campos que você mostrou no teu select
...
export default function VisitasPage() {
  ...
  async function carregarTudo() {
    ...
    try {
      ...
      // buscar km_lancamentos para estas visitas
      ...
      if (visitaIds.length > 0) {
        ...
        if (kmErr) {
          // não vamos quebrar página se esta tabela estiver com RLS/perm
          ...
          (kmData ?? []).forEach((k: any) => {
            // se houver mais de um por visita, você decide regra.
            // aqui pegamos o "último" que vier.
  ...
  function abrirModalEditar(visita: VisitaRow) {
    // Se REALIZADA, a regra do projeto é travar edição de km/campos no fluxo normal.
    // Admin ainda pode editar NOTAS se quiser, mas por enquanto travamos tudo (mais seguro).
    if ((visita.estado ?? 'AGENDADA') === 'REALIZADA') {
      alert('Visita REALIZADA não deve ser editada (auditoria).');
      return;
    }
    ...
    // Tentativa simples de reaproveitar notas existentes
    ...
    setForm({
      ...
      hora_inicio: '', // não existe no banco
      hora_fim: '', // não existe no banco
      objetivo: notas,
      resultado: '',
      proxima_acao: '',
      km_informado:
  ...
  function buildNotas() {
    // você pode mudar o formato depois, mas isto já funciona e é auditável
    const parts: string[] = [];
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // cliente_id pode ser null (visita sem cliente), mas aqui vamos exigir
    if (!form.cliente_id) {
  ...
  async function concluirVisita(visita: VisitaRow) {
    ...
    // regra: precisa de km_informado para criar lançamento
    ...
    try {
      ...
      // O trigger deve criar vendedor_km_lancamentos.
      // Recarregamos e mostramos no card.
      await carregarTudo();
                ...
                <div className="flex flex-row lg:flex-col gap-2 justify-end">
                  ...
                  {estado === 'REALIZADA' && (
                    <div className="text-xs text-gray-500 max-w-[220px]">
                      Esta visita está REALIZADA. Edição bloqueada para manter rastreabilidade.
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              {/* (opcionais, guardados nas notas também) */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Resultado (opcional)
              ...
              {/* Estado (admin pode ajustar antes de concluir via botão) */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Estado
                </label>
                ...
                >
                  ...
                  {/* REALIZADA deve ser via botão Concluir, para garantir trigger e auditoria */}
                </select>
                <p className="text-xs text-gray-500 mt-1">
                  REALIZADA é aplicada via botão “Concluir” para garantir rastreabilidade e o lançamento de KM.
                </p>
              </div>
